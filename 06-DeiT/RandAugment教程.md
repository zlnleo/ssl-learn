...
| 实验 | 配置 | test_acc (best) |
|---|---|---|
| M=0 | `--ra-n 0`（真正的无增强，= Baseline） | **63.27%** ✅ |
| M=5 | 轻度 | **65.47%** ✅ |
| M=9 | 论文默认 | **66.59%** ✅ |
| M=15 | 中度 | **67.16%** ✅ |
| M=20 | 重度 | **68.36%** ✅（**v2 最优**） |

> ✅ 五组已全部跑完（run_20260831_065657 ~ 100153，数据与 `m_ablation_results.txt`
> 交叉核对一致）。**发现：CIFAR-100 上只开 RandAugment 时，m 越大越好——到 M=20
> 曲线仍未拐头**，68.36% 甚至超过 CutMix 单用（67.90%）。这与论文在 ImageNet 上
> 调出 m=9 的经验相反：小图/小数据集需要更强的变换才能"逼"出不变性。
> 可留待回访：M=25/30 找拐点、CutMix+RA(M=20) 组合。

观察：**增强强度从弱到强，test_acc 如何变化**——你实测到的是一条**持续上升、尚未拐头**
的曲线（63.27 → 68.36，五个点严格单调）。这修正了教程原先"先升后平/后降"的预期：
拐点出现的位置取决于数据集，CIFAR 的拐点在 M=20 之后。这个实验让你真正理解
