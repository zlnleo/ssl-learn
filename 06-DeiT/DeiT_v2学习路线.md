...
| 4 | + RandAugment (n=2, m=9, inc=0) | 84.30% | 66.59% | 1.1948 | 2.0488 |
| 5 | + Mixup + CutMix | 61.97% | 67.12% | 1.6037 | 2.0166 |
| 6 | + 全部 (Mixup+CutMix+RA, inc=1) | 43.86% | 66.22% | 2.2431 | 1.9853 |

> 口径说明：test_acc 列为 **best**（最优轮）；train_acc/loss 为最后 epoch 值。
> 行 3 原填 67.59 是"最后一轮"的 0.6759，best 实为 67.90，已按 runs 日志修正。
> 行 4 数据来自 run_20260831_050356（早停于 81），行 6 来自 run_20260831_055411（跑满 100）。
...
### 7.1 全配方为何反而更低？（本轮最重要的发现）

单看 +3~4.6 的收益：Mixup +3.18、CutMix +4.63、RandAugment +3.32、Mixup+CutMix +3.85、
**全配方 +2.95**——全开反而**低于**单用 CutMix（67.90）和 Mixup+CutMix（67.12）。
这不是代码 bug，看 train_acc 就知道原因：

| 配置 | train_acc | 解读 |
|---|---|---|
| +RandAugment 单独 | 84.30% | 几何/色彩变换不破坏标签，正则相对温和 |
| +Mixup/CutMix | 61~72% | 样本混合破坏标签，正则更强 |
| 全配方 + **inc=1** | **43.86%** | 三重增强叠加 + 幅度爬到极限(30)，**正则过强 → 欠拟合** |

两个具体原因：
1. **递增幅度在 CIFAR 上过猛**：`m = 9 + (epoch-1)×1`，约 20 轮后实际幅度就被 clip 顶到
   30（极限强度），后半程模型吃的是"全强度三重增强"，学不动了；
2. **论文配方是为 ImageNet 调的**：128 万图 / 大模型 / 300 epoch 才扛得住这套强度；
   CIFAR 5 万图 + 5.4M 模型 + 100 epoch，全开即超载。

**这恰好证明了 M 消融实验的价值**（`RandAugment教程.md` 第 7 节）：强度不是越大越好，
存在一个"最佳旋钮位置"——而且 **M 消融已经跑完（2026-08-31），结果修正了这里的
预期**：只开 RA 时曲线持续上升，到 M=20 仍未拐头：

| M | 0 | 5 | 9 | 15 | 20 |
|---|---|---|---|---|---|
| best | 63.27% | 65.47% | 66.59% | 67.16% | **68.36%** |

**M=20 的 68.36% 是 v2 全场最优**，超过 CutMix 单用（67.90%）。结论更新：
- "全配方失败"的元凶是**三重叠加 + 递增幅度**，不是 RA 本身——RA 单独开时
  CIFAR 上 m 越大越好（与 ImageNet 经验相反，小图需要更强变换）；
- 左端点 M=0（`--ra-n 0`）精确等于 Baseline 63.27% ✓，消融链条干净。

### 7.2 状态更新（2026-08-31，按你的决定暂停 RA3/EMA）

- ✅ 已完成：M 消融五组（脚本 `run_m_ablation.py`，结果见上表与 `m_ablation_results.txt`）；
- ⏸ 暂停（回访 SSL/DINO 时再补）：重复增强 RA3、EMA、`--distill none` 蒸馏对照、
  Teacher quality、多 seed 平均；
- 🔜 回访时可选的延续实验：M=25/30 找拐点、CutMix+RA(M=20) 组合、全配方固定 m 重跑。
